...
- Patch versions (4.2.1 to 4.2.2) will make backwards-compatible bug fixes, or add small enhancements
...
You can change the URL Mastodon queries by setting the `UPDATE_CHECK_URL` environment variable. You can also completely disable this behaviour by setting this environment variable to an empty string. Note that we strongly recommend against doing so, unless you are keeping up with Mastodon updates in another way - Mastodon occasionally releases critical security updates that *must* be applied in a timely fashion.
...
Download the code of the release. For example, assuming that the version is called `v4.4.2`:
...
Now execute the upgrade instructions that are included in that version's release notes on GitHub. Different releases require different instructions, so we do not include any instructions on this page.
